...
@OptIn(ConsoleExperimentalApi::class)
object BusGroupCommand : CompositeCommand(
    ReporterPlugin,
    "bus_group", "BUS群组", // "manage" 是主指令名
    description = "BUS的群组白名单管理"
) {
    ...
    suspend fun CommandSender.list() {
        if (this is UserCommandSender) {
            listWhiteList(bot, user.asCommandSender(true), BusGroupWhiteList.groupIdsPerBot)
        } else if (this is ConsoleCommandSender) {
            for (bot in Bot.instances) {
                listWhiteList(bot, this, BusGroupWhiteList.groupIdsPerBot)
            }
        }
    }
    ...
    suspend fun CommandSender.add(target: Group) {
        if (this is UserCommandSender) {
            addWhiteList(
                ...
                user.asCommandSender(true),
                target,
                BusGroupWhiteList.groupIdsPerBot
            )
        } else if (this is ConsoleCommandSender) {
            for (bot in Bot.instances) {
                addWhiteList(
    ...
    suspend fun CommandSender.remove(target: Group) {
        if (this is UserCommandSender) {
            removeWhiteList(bot, user.asCommandSender(true), target, BusGroupWhiteList.groupIdsPerBot)
        } else {
            for (bot in Bot.instances) {
                removeWhiteList(bot, this, target, BusGroupWhiteList.groupIdsPerBot)
            }
        }
    }
}
...
object BusGroupWhiteList : AutoSavePluginConfig("busGroupWhiteList") {
    val groupIdsPerBot: MutableMap<Long, MutableList<Long>> by value()
}
